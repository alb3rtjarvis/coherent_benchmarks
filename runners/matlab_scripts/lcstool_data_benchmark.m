function lcstool_data_benchmark(run_config_json)
% Benchmark script for numerical velocity example
%   run_config_json: json file containing the following keys
%   keys listed below
    %   flow_data: struct containing flow data
    ...
    vel_data_paths = flow_data.vel_data_paths;
    t0 = flow_data.t0;
    T = flow_data.T;
    dt0 = flow_data.dt0;
    fdomain = flow_data.domain;
    ...
    %disp(['Flow data path: ', vel_data_paths]);
    ...
    % Input parameters
    u = double(py.numpy.load(vel_data_paths.u));
    v = double(py.numpy.load(vel_data_paths.v));
    dimorder = [1, 3, 2];
    u = permute(u, dimorder);
    v = permute(v, dimorder);
    [nt, resolutionY, resolutionX] = size(u);
    domain = [fdomain(2,1), fdomain(2,2); fdomain(3,1), fdomain(3,2)];
    time = linspace(domain(1,1), domain(1,2), nt);
    [~,deltaX] = equal_resolution(domain,resolutionX);
    resolution = [resolutionX,resolutionY];
    xd = linspace(domain(1,1), domain(1,2), resolutionX);
    yd = linspace(domain(2,1), domain(2,2), resolutionY);
    timespan = [t0,T];
    ...
    v(:,[1,end],:) = 0;
    v(:,:,[1,end]) = 0;
    interpMethod = 'cubic'; % switch from spline for better memory handling
    uInterpolant = griddedInterpolant({time,yd,xd},u,interpMethod);
    vInterpolant = griddedInterpolant({time,yd,xd},v,interpMethod);
    lDerivative = @(t,x,~)data_derivative(t,x,uInterpolant,vInterpolant);
    incompressible = true;
